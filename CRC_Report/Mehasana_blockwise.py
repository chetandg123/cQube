...
class Gothva_cluster(unittest.TestCase):
    ...
    def test_crcclick(self):
        ...
        time.sleep(5)
        blk = self.driver.find_element_by_xpath("//select[@name='myBlock']/option[contains(text(),' Jotana ')]").click()
        block = self.driver.find_element_by_xpath("//select[@name='myBlock']/option[contains(text(),' Jotana ')]").text
        print(block)
        self.assertEqual(" Jotana ",block,"is not selected ")
        time.sleep(5)
        # clu = self.driver.find_element_by_xpath("//select[@name='myCluster']/option[contains(text(),' Gothva ')]").text
        # print(clu)
        # self.assertEqual(" Gothva ", block, "is not selected ")
        # time.sleep(3)
        ...
        with open("/home/chetan/Downloads/Datafiles/Cluster_level_CRC_Report (4).csv", "r") as file:
