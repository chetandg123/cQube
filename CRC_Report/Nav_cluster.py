...
class Nav_cluster(unittest.TestCase):
    ...
    def test_crcclick(self):
        ...
        blk = self.driver.find_element_by_xpath("//select[@name='myBlock']/option[contains(text(),'Navsari')]").click()
        block = self.driver.find_element_by_xpath("//select[@name='myBlock']/option[contains(text(),'Navsari')]").text
        print(block)
        self.assertEqual(" Navsari ",block,"is not selected ")
        time.sleep(5)
        self.driver.find_element_by_xpath("//select[@name='myCluster']/option[contains(text(),'Khadsupa')]").click()
        clu = self.driver.find_element_by_xpath("//select[@name='myCluster']/option[contains(text(),'Khadsupa')]").text
        print(clu)
        self.assertEqual(" Khadsupa ",clu,"cluster is not loaded")
        time.sleep(5)
        ...
        with open("/home/chetan/Downloads/Datafiles/School_level_CRC_Report (2).csv", "r") as file:
