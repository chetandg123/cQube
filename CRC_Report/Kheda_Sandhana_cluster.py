...
class Sandhana_cluster(unittest.TestCase):
    def setUp(self):
    ...
    def test_crcclick(self):
        ...
        time.sleep(5)
        blk = self.driver.find_element_by_xpath("//select[@name='myBlock']/option[contains(text(),'Matara')]").click()
        block = self.driver.find_element_by_xpath("//select[@name='myBlock']/option[contains(text(),'Matara')]").text
        print(block)
        self.assertEqual(" Matara ",block,"is not selected ")
        time.sleep(5)
        self.driver.find_element_by_xpath("//select[@name='myCluster']/option[contains(text(),'Sandhana')]").click()
        clu = self.driver.find_element_by_xpath("//select[@name='myCluster']/option[contains(text(),'Sandhana')]").text
        print(clu)
        self.assertEqual(" Sandhana ",clu,"cluster is not loaded")
        time.sleep(5)
